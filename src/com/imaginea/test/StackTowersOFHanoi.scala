...
object StackTowersOFHanoi {
  def main(args: Array[String]): Unit = {
    ...
    do {
      ...
      if (k == 1) {
        status = checkMove(sourceStack, destinationStack),x
        
//        val arr= Array(checkMove(sourceStack, destinationStack),x)
        if ( status == 1) {
         stepNum=stepNum+1
         
          println("Disk "+v+" moved from Source Tower to Destination Tower ")
        } else if(status ==2){
         stepNum=stepNum+1
          println("Disk "+v+"  moved from Destination Tower to Source Tower ")
        }
        
        status = checkMove(sourceStack, pivotStack)
        if ( status == 1) {
     stepNum=stepNum+1
          println("Disk "+v+" moved from Source Tower to Pivot Tower ")
        } else if(status ==2){
         stepNum=stepNum+1
          println("Disk "+v+" moved from Pivot Tower to Source Tower ")
        }else{
         return
        }
      } else {
      ...
      status = checkMove(pivotStack, destinationStack)
        if ( status == 1) {
           stepNum=stepNum+1
          println("Disk "+stepNum+" moved from Pivot Tower to Destination Tower ")
        } else if(status == 2){
           stepNum=stepNum+1
          println("Disk "+stepNum+" moved from Destination Tower to Pivot Tower ")
        }
  ...
  def checkMove(srcStack: Stack[Int], destStack: Stack[Int]): (Int,Int) = {
...
//        println("Empty Stack")
    ...
    if (i == j) return (0,i.toInt) //both towers are empty
